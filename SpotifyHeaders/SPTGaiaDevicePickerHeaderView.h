//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

#import "SPTGaiaDevicePickerHeader.h"
#import "SPTThemableView.h"

@class NSLayoutConstraint, NSNotificationCenter, NSString, SPTLayoutConstraintBuilder, SPTTheme, UIImageView, UILabel;

@interface SPTGaiaDevicePickerHeaderView : UIView <SPTThemableView, SPTGaiaDevicePickerHeader>
{
    id <SPTThemableViewLayoutDelegate> _layoutDelegate;
    unsigned long long _state;
    long long _themeStyle;
    UILabel *_titleLabel;
    SPTTheme *_theme;
    NSNotificationCenter *_notificationCenter;
    UIImageView *_imageView;
    SPTLayoutConstraintBuilder *_imageViewLayout;
    SPTLayoutConstraintBuilder *_titleLayout;
    UILabel *_subtitleLabel;
    SPTLayoutConstraintBuilder *_subtitleLayout;
    NSLayoutConstraint *_subtitleHeightConstraint;
}

@property(retain, nonatomic) NSLayoutConstraint *subtitleHeightConstraint; // @synthesize subtitleHeightConstraint=_subtitleHeightConstraint;
@property(retain, nonatomic) SPTLayoutConstraintBuilder *subtitleLayout; // @synthesize subtitleLayout=_subtitleLayout;
@property(retain, nonatomic) UILabel *subtitleLabel; // @synthesize subtitleLabel=_subtitleLabel;
@property(retain, nonatomic) SPTLayoutConstraintBuilder *titleLayout; // @synthesize titleLayout=_titleLayout;
@property(retain, nonatomic) SPTLayoutConstraintBuilder *imageViewLayout; // @synthesize imageViewLayout=_imageViewLayout;
@property(retain, nonatomic) UIImageView *imageView; // @synthesize imageView=_imageView;
@property(retain, nonatomic) NSNotificationCenter *notificationCenter; // @synthesize notificationCenter=_notificationCenter;
@property(retain, nonatomic) SPTTheme *theme; // @synthesize theme=_theme;
@property(retain, nonatomic) UILabel *titleLabel; // @synthesize titleLabel=_titleLabel;
@property(nonatomic) long long themeStyle; // @synthesize themeStyle=_themeStyle;
@property(nonatomic) unsigned long long state; // @synthesize state=_state;
@property(nonatomic) __weak id <SPTThemableViewLayoutDelegate> layoutDelegate; // @synthesize layoutDelegate=_layoutDelegate;
- (void).cxx_destruct;
- (void)applyThemeLayout;
- (void)addImageViewLayoutConstraints;
- (void)configureImageViewAndAddAsSubview;
- (void)updateSubtitleHeightConstraint;
- (void)addSubtitleLabelLayoutConstraints;
- (void)configureSubtitleLabelAndAddAsSubview;
- (void)addTitleLabelLayoutConstraints;
- (void)configureTitleLabelAndAddAsSubview;
- (void)dealloc;
- (id)initWithFrame:(struct CGRect)arg1 theme:(id)arg2 notificationCenter:(id)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

