//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "SOJUSnapStoryDoublePostResponse-Protocol.h"

@class NSArray, NSDictionary, NSNumber, NSString, SOJUJsonStoryResponse, SOJUSecurityFideliusFriendsKeysResponse, SOJUServerInfoResponse;

@interface SOJUSnapStoryDoublePostResponse : NSObject <SOJUSnapStoryDoublePostResponse>
{
    SOJUServerInfoResponse *_serverInfo;
    SOJUJsonStoryResponse *_json;
    NSArray *_groupStories;
    NSArray *_verifiedStories;
    NSArray *_verifiedStoriesWithCollabs;
    NSArray *_myMobStories;
    NSNumber *_success;
    NSDictionary *_snaps;
    NSDictionary *_inviteSnaps;
    NSDictionary *_snapstreakData;
    SOJUSecurityFideliusFriendsKeysResponse *_fideliusFriendKeys;
    NSNumber *_sendKey;
    NSNumber *_recipientFidReady;
}

+ (unsigned long long *)fasterCodingKeys;
+ (unsigned long long)fasterCodingVersion;
+ (_Bool)canInitFromProto;
@property(readonly, copy, nonatomic) NSNumber *recipientFidReady; // @synthesize recipientFidReady=_recipientFidReady;
@property(readonly, copy, nonatomic) NSNumber *sendKey; // @synthesize sendKey=_sendKey;
@property(readonly, copy, nonatomic) SOJUSecurityFideliusFriendsKeysResponse *fideliusFriendKeys; // @synthesize fideliusFriendKeys=_fideliusFriendKeys;
@property(readonly, copy, nonatomic) NSDictionary *snapstreakData; // @synthesize snapstreakData=_snapstreakData;
@property(readonly, copy, nonatomic) NSDictionary *inviteSnaps; // @synthesize inviteSnaps=_inviteSnaps;
@property(readonly, copy, nonatomic) NSDictionary *snaps; // @synthesize snaps=_snaps;
@property(readonly, copy, nonatomic) NSNumber *success; // @synthesize success=_success;
@property(readonly, copy, nonatomic) NSArray *myMobStories; // @synthesize myMobStories=_myMobStories;
@property(readonly, copy, nonatomic) NSArray *verifiedStoriesWithCollabs; // @synthesize verifiedStoriesWithCollabs=_verifiedStoriesWithCollabs;
@property(readonly, copy, nonatomic) NSArray *verifiedStories; // @synthesize verifiedStories=_verifiedStories;
@property(readonly, copy, nonatomic) NSArray *groupStories; // @synthesize groupStories=_groupStories;
@property(readonly, copy, nonatomic) SOJUJsonStoryResponse *json; // @synthesize json=_json;
@property(readonly, copy, nonatomic) SOJUServerInfoResponse *serverInfo; // @synthesize serverInfo=_serverInfo;
- (void).cxx_destruct;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
- (_Bool)isEqual:(id)arg1;
- (void)setObject:(id)arg1 forUInt64Key:(unsigned long long)arg2;
- (void)decodeWithFasterDecoder:(id)arg1;
- (void)encodeWithFasterCoder:(id)arg1;
- (_Bool)preferFasterCoding;
- (void)encodeWithCoder:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)initWithServerInfo:(id)arg1 json:(id)arg2 groupStories:(id)arg3 verifiedStories:(id)arg4 verifiedStoriesWithCollabs:(id)arg5 myMobStories:(id)arg6 success:(id)arg7 snaps:(id)arg8 inviteSnaps:(id)arg9 snapstreakData:(id)arg10 fideliusFriendKeys:(id)arg11 sendKey:(id)arg12 recipientFidReady:(id)arg13;
- (id)toJson;
- (id)toDictionary;
- (id)initWithJSONDictionary:(id)arg1;
- (_Bool)recipientFidReadyValue;
- (_Bool)sendKeyValue;
- (_Bool)successValue;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly) Class superclass;

@end

