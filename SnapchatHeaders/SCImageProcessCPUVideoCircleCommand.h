//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "SCImageProcessCPUCommand.h"

@class CIContext, CIFilter;

@interface SCImageProcessCPUVideoCircleCommand : SCImageProcessCPUCommand
{
    struct CGColorSpace *_colorSpace;
    CIContext *_context;
    CIFilter *_filter;
}

+ (id)commandWithOutputSize:(struct CGSize)arg1 radius:(id)arg2 color:(struct CGColor *)arg3 mask:(_Bool)arg4;
@property(retain, nonatomic) CIFilter *filter; // @synthesize filter=_filter;
@property(retain, nonatomic) CIContext *context; // @synthesize context=_context;
- (void).cxx_destruct;
- (id)runWithContext:(id)arg1 inputPixelBuffer:(struct __CVBuffer *)arg2 outputPixelBuffer:(struct __CVBuffer *)arg3 orientation:(long long)arg4;
- (void)dealloc;
- (id)initWithOutputSize:(struct CGSize)arg1 radius:(id)arg2 color:(struct CGColor *)arg3 mask:(_Bool)arg4;

@end

