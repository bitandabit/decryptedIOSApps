//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "SCOperaGifProvider-Protocol.h"
#import "SCOperaImageProvider-Protocol.h"

@class SCBaseMediaMessage;

@interface SCBaseMediaMessageImageProvider : NSObject <SCOperaGifProvider, SCOperaImageProvider>
{
    SCBaseMediaMessage *_baseMediaMessage;
}

- (void).cxx_destruct;
- (void)gifForKey:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (void)imageForKey:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (id)initWithBaseMediaMessage:(id)arg1;

@end

