//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "SCExperimentContext.h"

@interface SCLagunaMalibuExperimentContext : SCExperimentContext
{
    _Bool _enableMalibu;
    _Bool _enableHEVC;
    _Bool _enableHomeWiFi;
    _Bool _enableHomeWiFiInstructions;
}

@property(readonly, nonatomic) _Bool enableHomeWiFiInstructions; // @synthesize enableHomeWiFiInstructions=_enableHomeWiFiInstructions;
@property(readonly, nonatomic) _Bool enableHomeWiFi; // @synthesize enableHomeWiFi=_enableHomeWiFi;
@property(readonly, nonatomic) _Bool enableHEVC; // @synthesize enableHEVC=_enableHEVC;
@property(readonly, nonatomic) _Bool enableMalibu; // @synthesize enableMalibu=_enableMalibu;
- (void)setupParameters;
- (id)experimentName;

@end

