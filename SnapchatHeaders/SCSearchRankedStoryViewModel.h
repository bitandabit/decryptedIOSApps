//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "NSCopying-Protocol.h"

@class NSString, SCSearchActionModel, SCSearchSnapMedia;

@interface SCSearchRankedStoryViewModel : NSObject <NSCopying>
{
    NSString *_title;
    NSString *_subtitle;
    NSString *_geoText;
    NSString *_debugInfo;
    SCSearchSnapMedia *_thumbnail;
    SCSearchActionModel *_tapActionModel;
}

@property(readonly, copy, nonatomic) SCSearchActionModel *tapActionModel; // @synthesize tapActionModel=_tapActionModel;
@property(readonly, copy, nonatomic) SCSearchSnapMedia *thumbnail; // @synthesize thumbnail=_thumbnail;
@property(readonly, copy, nonatomic) NSString *debugInfo; // @synthesize debugInfo=_debugInfo;
@property(readonly, copy, nonatomic) NSString *geoText; // @synthesize geoText=_geoText;
@property(readonly, copy, nonatomic) NSString *subtitle; // @synthesize subtitle=_subtitle;
@property(readonly, copy, nonatomic) NSString *title; // @synthesize title=_title;
- (void).cxx_destruct;
- (_Bool)isEqual:(id)arg1;
- (unsigned long long)hash;
- (id)description;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)initWithTitle:(id)arg1 subtitle:(id)arg2 geoText:(id)arg3 debugInfo:(id)arg4 thumbnail:(id)arg5 tapActionModel:(id)arg6;

@end

