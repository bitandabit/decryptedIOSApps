//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@class NSString;

@interface ADLUserStateChangedEvent : NSObject
{
    _Bool _isConnected;
    _Bool _audioPublished;
    _Bool _videoPublished;
    _Bool _screenPublished;
    NSString *_scopeId;
    long long _userId;
    long long _mediaType;
    NSString *_videoSinkId;
    NSString *_screenSinkId;
}

+ (id)UserStateChangedEventWithScopeId:(id)arg1 userId:(long long)arg2 isConnected:(_Bool)arg3 mediaType:(long long)arg4 audioPublished:(_Bool)arg5 videoPublished:(_Bool)arg6 videoSinkId:(id)arg7 screenPublished:(_Bool)arg8 screenSinkId:(id)arg9;
@property(readonly, nonatomic) NSString *screenSinkId; // @synthesize screenSinkId=_screenSinkId;
@property(readonly, nonatomic) _Bool screenPublished; // @synthesize screenPublished=_screenPublished;
@property(readonly, nonatomic) NSString *videoSinkId; // @synthesize videoSinkId=_videoSinkId;
@property(readonly, nonatomic) _Bool videoPublished; // @synthesize videoPublished=_videoPublished;
@property(readonly, nonatomic) _Bool audioPublished; // @synthesize audioPublished=_audioPublished;
@property(readonly, nonatomic) long long mediaType; // @synthesize mediaType=_mediaType;
@property(readonly, nonatomic) _Bool isConnected; // @synthesize isConnected=_isConnected;
@property(readonly, nonatomic) long long userId; // @synthesize userId=_userId;
@property(readonly, nonatomic) NSString *scopeId; // @synthesize scopeId=_scopeId;
- (void).cxx_destruct;
- (id)description;
- (id)initWithScopeId:(id)arg1 userId:(long long)arg2 isConnected:(_Bool)arg3 mediaType:(long long)arg4 audioPublished:(_Bool)arg5 videoPublished:(_Bool)arg6 videoSinkId:(id)arg7 screenPublished:(_Bool)arg8 screenSinkId:(id)arg9;

@end

