//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "SCCloudFSFileEntity-Protocol.h"

@class NSString, SCDataObjectContext, SCMediaDownload;
@protocol SCGallerySnap, SCSyncFileResolver;

@interface SCCloudFSHDMediaEntity : NSObject <SCCloudFSFileEntity>
{
    NSString *_UUID;
    id <SCGallerySnap> _snap;
    id <SCSyncFileResolver> _fileResolver;
    SCMediaDownload *_mediaDownload;
    SCDataObjectContext *_dataObjectContext;
}

- (void).cxx_destruct;
- (double)cacheTTL;
- (id)entityName;
- (id)downloadWithMediaScenePath:(CDStruct_bac8f6e9)arg1 resultHandler:(CDUnknownBlockType)arg2;
- (id)allRepresentations;
- (id)UUID;
- (id)initWithSnap:(id)arg1 mediaDownload:(id)arg2 fileResolver:(id)arg3 dataObjectContext:(id)arg4;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

